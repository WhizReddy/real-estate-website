...
export default function PropertyDetailMap({
  ...
  const addPropertyMarker = (L: any, map: any, prop: Property) => {
    ...
    const mainMarkerIcon = L.divIcon({
      html: `
  <div class="main-property-marker bg-blue-600 text-white px-4 py-3 rounded-lg text-sm font-bold shadow-xl border-3 border-white relative">
  ...
  const addNearbyPropertyMarkers = (L: any, map: any, properties: Property[]) => {
    properties.forEach((prop) => {
      ...
      const nearbyMarkerIcon = L.divIcon({
        html: `
          <div class="nearby-property-marker bg-white text-blue-700 px-3 py-2 rounded-lg text-xs font-semibold shadow-lg border-2 border-blue-200 transition-all cursor-pointer">
            ${formatPrice(prop.price)}
          </div>
        `,
